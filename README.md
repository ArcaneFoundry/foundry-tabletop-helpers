...
- Class, Origins, and Build now run inside a mounted React shell that keeps the window chrome and progress rail resident while only the content pane changes
...
- Build now runs under a dedicated `Build` milestone with mounted ability assignment, source-aware class/background equipment selection, a conditional mundane shop, and the remaining feat/spell steps still hosted inside the same shell
...
- Equipment selection now distinguishes class and background provisions, supports equipment-vs-gold decisions per source, respects mundane-only and firearm rules, and can hand remaining currency into a buy/sell shop before actor creation
...
Contributor-oriented internal docs now live in the project vault. Use [docs/README.md](/Users/johngallego/CodeProjects/foundry-tabletop-helpers/docs/README.md) for the current internal doc index.
