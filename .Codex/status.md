...
- Inspected the current monster preview implementation and selected the first high-ROI slice intentionally: improve the "Up Next" preview to skip defeated combatants and show more combat-relevant HP information.
- Completed the first Phase 4 Combat Monster Preview slice by extracting up-next selection into `src/combat/monster-preview/monster-preview-up-next.ts`.
- Updated the monster preview panel and rendering so "Up Next" now skips defeated combatants and shows current/max HP for NPCs when available instead of only max HP.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-up-next.test.ts` and extended rendering coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts`.
- Verified the slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on Combat Monster Preview live refresh behavior so the active NPC panel can stay current after HP or condition changes during its turn.
- Completed the live-refresh slice by adding a debounced monster preview refresh controller in `src/combat/monster-preview/monster-preview-live-refresh.ts`.
- Rewired `src/combat/monster-preview/monster-preview-panel.ts` to listen for `updateActor` plus active-effect create/update/delete hooks and refresh the currently displayed NPC preview when its HP or conditions change mid-turn.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-live-refresh.test.ts`.
- Verified the live-refresh slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on monster preview interaction affordances: opening the active monster sheet from the preview and opening the up-next combatant from the up-next row.
- Completed the interaction-affordance slice by making the active monster identity and up-next row open actor sheets directly from the preview.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts`, `src/combat/monster-preview/monster-preview-interactions.ts`, and `src/combat/monster-preview/monster-preview-panel.ts` to render/open those actor-sheet affordances in both inline and floating modes.
- Added regression coverage for the new clickable preview flows in `src/combat/monster-preview/monster-preview-rendering.test.ts`, `src/combat/monster-preview/monster-preview-interactions.test.ts`, and `src/combat/monster-preview/monster-preview-up-next.test.ts`.
- Verified the interaction slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on a compact monster preview status strip for defeated and active-condition badges near the identity block.
- Completed the compact status-strip slice by adding defeated, concentrating, and condition badge extraction in `src/combat/monster-preview/monster-preview-status.ts`.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts`, `src/combat/monster-preview/monster-preview-panel.ts`, and `src/combat/styles/combat-monster-preview.css` to display the new compact status badges directly under the monster identity.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-status.test.ts` and extended rendering coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts`.
- Verified the status-strip slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on compact monster preview quick actions for common GM actions directly from the preview panel.
- Completed the quick-action slice by adding a compact action row for sheet open, initiative, Perception, Stealth, and Wis Save actions directly in the preview.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts`, `src/combat/monster-preview/monster-preview-interactions.ts`, `src/combat/monster-preview/monster-preview-panel.ts`, and `src/combat/styles/combat-monster-preview.css` to render, wire, and execute those common preview actions in both inline and floating modes.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts` and `src/combat/monster-preview/monster-preview-interactions.test.ts`.
- Verified the quick-action slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on monster preview encounter-context cues so duplicate monsters are easier to distinguish at a glance.
- Completed the encounter-context slice by adding token-name, initiative, and round/turn cue extraction in `src/combat/monster-preview/monster-preview-context.ts`.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts`, `src/combat/monster-preview/monster-preview-panel.ts`, and `src/combat/styles/combat-monster-preview.css` to display compact encounter-context chips directly in the monster identity block.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-context.test.ts` and extended rendering coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts`.
- Verified the encounter-context slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on a minimized floating monster preview mode so GMs can keep key context visible with less screen space.
- Completed the minimized floating-mode slice by adding a persisted minimized state for the floating monster preview panel.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts`, `src/combat/monster-preview/monster-preview-interactions.ts`, `src/combat/monster-preview/monster-preview-panel.ts`, `src/combat/monster-preview/monster-preview-floating.ts`, and `src/combat/styles/combat-monster-preview.css` to render, toggle, persist, and style the minimized preview mode.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts` and `src/combat/monster-preview/monster-preview-interactions.test.ts`.
- Verified the minimized-mode slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on a GM-facing default display setting for the monster preview.
- Completed the default-display slice by adding `monsterPreviewDefaultDisplay` to `src/combat/combat-settings.ts` with world-configured choices for Remember Last, Inline, Floating, and Floating (Minimized).
- Added startup display-state initialization in `src/combat/monster-preview/monster-preview-display.ts` and rewired `src/combat/monster-preview/monster-preview-panel.ts` to seed preview mode/minimized state from the GM-facing setting.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-display.test.ts`.
- Verified the default-display slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on a reset/clear affordance for monster preview local layout state.
- Completed the layout-reset slice by adding a floating-panel reset affordance for monster preview local layout state.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts`, `src/combat/monster-preview/monster-preview-interactions.ts`, `src/combat/monster-preview/monster-preview-panel.ts`, `src/combat/monster-preview/monster-preview-floating.ts`, and `src/combat/styles/combat-monster-preview.css` to render, wire, and execute the reset-layout action while restoring the floating preview to its default expanded position.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts`, `src/combat/monster-preview/monster-preview-interactions.test.ts`, and `src/combat/monster-preview/monster-preview-floating.test.ts`.
- Verified the layout-reset slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on a GM-facing setting for which monster preview quick actions appear.
- Completed the configurable quick-actions slice by adding `monsterPreviewQuickActions` to `src/combat/combat-settings.ts` as a world-configured quick-action list with support for hiding the row entirely.
- Added quick-action setting resolution in `src/combat/monster-preview/monster-preview-quick-actions.ts` and rewired `src/combat/monster-preview/monster-preview-panel.ts` plus `src/combat/monster-preview/monster-preview-rendering.ts` to render only the configured quick-action subset.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-quick-actions.test.ts` and extended rendering coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts`.
- Verified the configurable quick-actions slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Deferred the compact monster preview copy/export affordance and recorded it as GitHub backlog issue `#14` (`Backlog: add monster preview copy/export affordance`).
- Completed the quick-action UX-polish slice by moving monster preview quick-action selection from a raw string setting to a dedicated GM-facing submenu with checkbox controls.
- Updated `src/combat/combat-settings.ts`, `src/combat/monster-preview/monster-preview-quick-actions.ts`, and `templates/combat/monster-preview-quick-actions.hbs` so GMs can configure the preview quick-action subset through a native settings menu while preserving stable compact ordering and support for hiding the row entirely.
- Added behavior-focused regression coverage in `src/combat/combat-settings.test.ts` and extended `src/combat/monster-preview/monster-preview-quick-actions.test.ts`.
- Verified the quick-action UX-polish slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on a compact active-turn emphasis treatment in the monster preview identity block.
- Completed the active-turn emphasis slice by adding a stronger active identity treatment and banner pill when the previewed monster is the currently acting combatant.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts` and `src/combat/styles/combat-monster-preview.css` so the acting monster reads more clearly in both full and minimized preview modes without changing the underlying combat context model.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts`.
- Verified the active-turn emphasis slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Deferred the richer up-next NPC differentiation idea and recorded it as GitHub backlog issue `#15` (`Backlog: add richer up-next monster preview cues`).
- Started the next Phase 4 slice on a GM-facing monster preview availability toggle.
- Completed the availability-toggle slice by adding a world setting to keep the last monster preview visible between turns instead of always auto-hiding on non-NPC turns.
- Updated `src/combat/combat-settings.ts`, `src/combat/monster-preview/monster-preview-panel.ts`, and `src/combat/monster-preview/monster-preview-availability.ts` to register, evaluate, and apply the new between-turn visibility behavior while preserving the default NPC-turn-only flow.
- Added behavior-focused regression coverage in `src/combat/combat-settings.test.ts` and `src/combat/monster-preview/monster-preview-availability.test.ts`.
- Verified the availability-toggle slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on an explicit pin/keep-open affordance for monster preview.
- Completed the pin/keep-open slice by adding a local pin toggle to the monster preview header in both inline and floating modes.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts`, `src/combat/monster-preview/monster-preview-interactions.ts`, `src/combat/monster-preview/monster-preview-panel.ts`, `src/combat/monster-preview/monster-preview-tracker.ts`, `src/combat/monster-preview/monster-preview-floating.ts`, `src/combat/monster-preview/monster-preview-availability.ts`, and `src/combat/styles/combat-monster-preview.css` so GMs can pin the preview open between turns without relying only on the world-level setting.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts`, `src/combat/monster-preview/monster-preview-interactions.test.ts`, `src/combat/monster-preview/monster-preview-tracker.test.ts`, `src/combat/monster-preview/monster-preview-floating.test.ts`, and `src/combat/monster-preview/monster-preview-availability.test.ts`.
- Verified the pin/keep-open slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on a persistence-state cue in the monster preview header.
- Completed the header-cue slice by adding a compact header badge that explains when the preview is staying visible because it is pinned or world-persistent.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts`, `src/combat/monster-preview/monster-preview-panel.ts`, `src/combat/monster-preview/monster-preview-tracker.ts`, and `src/combat/styles/combat-monster-preview.css` so the persistence reason is visible without adding noisy UI during normal active-turn preview behavior.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts` and extended tracker rendering coverage in `src/combat/monster-preview/monster-preview-tracker.test.ts`.
- Verified the header-cue slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Started the next Phase 4 slice on transient in-panel feedback for monster preview state changes.
- Completed the transient-feedback slice by adding a short-lived header flash for pin, unpin, dock, and layout-reset actions.
- Updated `src/combat/monster-preview/monster-preview-rendering.ts`, `src/combat/monster-preview/monster-preview-tracker.ts`, `src/combat/monster-preview/monster-preview-panel.ts`, and `src/combat/styles/combat-monster-preview.css` so those preview state changes feel explicit without relying on global notifications.
- Added behavior-focused regression coverage in `src/combat/monster-preview/monster-preview-rendering.test.ts`.
- Verified the transient-feedback slice with `npm run typecheck`, `npm run test`, and `npm run build`.
- Completed the persistence UX-smoothing slice by clarifying the difference between local pinning and world-level between-turn persistence in both settings copy and preview labels/tooltips.
- Updated `src/combat/combat-settings.ts` plus `src/combat/monster-preview/monster-preview-rendering.ts` so the world settings explain the persistence model more directly and the preview labels read as `Pinned Here` vs `World Persistent`.
- Added regression coverage in `src/combat/combat-settings.test.ts` and extended `src/combat/monster-preview/monster-preview-rendering.test.ts`.
- Verified the persistence UX-smoothing slice with `npm run typecheck`, `npm run test`, and `npm run build`.

## Next Suggested Steps

- Next highest-value Phase 4 target: assess whether Phase 4 is at a good stopping point for Combat Monster Preview polish, or else do one final very small pass on discoverability such as a brief tooltip/help note for the pin control in minimized mode.
