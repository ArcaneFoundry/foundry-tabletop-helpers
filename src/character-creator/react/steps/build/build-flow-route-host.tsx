    ...
    <section className="flex flex-col px-4 py-5 md:px-6 md:py-6">
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-4 rounded-[1.5rem] border border-white/10 bg-[linear-gradient(180deg,rgba(28,28,34,0.96),rgba(17,17,22,0.98))] p-5 shadow-[0_24px_60px_rgba(0,0,0,0.34)]">
        <div className="space-y-2">
          ...
          <h2 className="m-0 font-fth-cc-display text-[1.5rem] leading-none text-[#f7e7c6]">
            {title || "Build Choices"}
          </h2>
          <p className="m-0 max-w-3xl font-fth-cc-body text-[0.98rem] leading-7 text-[#d7d0cb]">
...
function SummaryCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[1.15rem] border border-white/10 bg-black/20 px-4 py-3">
      <div className="font-fth-cc-ui text-[0.62rem] uppercase tracking-[0.18em] text-[#e9c176]/68">
        {label}
      </div>
      <div className="mt-2 font-fth-cc-body text-[0.98rem] leading-6 text-[#f2ebe2]">
        {value}
      </div>
    </div>
  );
